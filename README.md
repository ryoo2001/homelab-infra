...
这个仓库记录 HomeLab 的架构设计、服务分层、安全边界和运维流程。仓库中的配置均为脱敏样例，不包含真实密钥、后台入口、内网地址表或 Tailscale 设备信息。
...
| 场景 | 链路 | 适合服务 |
| --- | --- | --- |
| 公开访问 | Internet -> Cloudflare -> Tunnel -> cloudflared -> reverse-nginx -> public services | 博客前台、公开状态页 |
...
本仓库保留 `yuuyan.top` 作为公开访问域名示例，但隐藏以下信息：
