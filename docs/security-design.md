...
HomeLab 的安全目标是减少公网暴露面，并把不同风险等级的服务放在不同访问边界内。

## 服务分级

| 等级 | 服务类型 | 访问方式 |
| --- | --- | --- |
| 公开展示 | 博客前台、公开状态页 | `yuuyan.top` 子域名 |
...
Cloudflare Access 适合保护半公开入口，例如：

- 仅自己需要偶尔远程访问的页面。
- 不适合放到 Tailscale 但也不应完全公开的服务。

Access 策略不应在公开仓库中完整展示。仓库只保留概念说明，不保存真实邮箱、组织策略或认证细节。
...
这些命令用于发现可能遗留的敏感内容。命中结果需要人工判断，示例占位符可以保留，真实凭据和真实私有地址不能保留。
